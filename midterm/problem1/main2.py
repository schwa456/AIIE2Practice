...
def __main__():
    ...
    print("LightGBM Regressor has been created")
    ...
    lgb_regressor.grid_search(X, y, param_grid)

    loocv_lgb = LOOCV(lgb_regressor)
    print("LOOCV for LightGBM Regressor has been created")
    loocv_lgb.fit(X, y)
    ...
    for metric in metrics:
        print(f"LOOCV of Light GBM Regression using {metric.upper()} : {loocv_lgb.mean_score(metric):.4f}")
    print(f"Light GBM Score for R Squared : {lgb_regressor.score(X, y)[2]:.4f}")
